...
async function create(owner, projectId, options = {}) {
  const args = ['project', 'field-create', projectId, '--owner', owner, '--name', options.name, '--type', options.type];

  if (options.options && Array.isArray(options.options)) {
    args.push('--options', options.options.join(','));
  }
 ...
 */
async function removeField(fieldId, projectId = null) {
  const args = ['project', 'field-delete', fieldId];
