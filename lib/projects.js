...
async function create(owner, title, options = {}) {
  ...
  const { stdout } = await execa('gh', createArgs, { reject: false });
  console.error('DEBUG create stdout:', stdout); // debug

  let data;
  try {
    data = JSON.parse(stdout);
  } catch (e) {
    throw new Error(`Failed to parse create output: ${stdout}`);
  }

  const projectId = data.number;

  // If description provided, set it via edit (gh project create doesn't support description)
...
async function view(owner, projectId) {
  ...
  const { stdout, stderr } = await execa('gh', args, { reject: false });
  console.error('DEBUG view stdout:', stdout);
  console.error('DEBUG view stderr:', stderr);
  let data;
  try {
    data = typeof stdout === 'string' ? JSON.parse(stdout) : stdout;
  } catch (e) {
    throw new Error(`Failed to parse view output: ${stdout}`);
  }
