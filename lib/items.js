 ...
 * @param {string} owner - GitHub owner (NOT used, kept for CLI compatibility)
 * @param {string} projectId - Project ID (NOT used, kept for CLI compatibility)
 * @param {string} itemId - Item ID
 */
async function removeItem(owner, projectId, itemId) {
  const args = ['project', 'item-delete', '--id', itemId];
  await execa('gh', args);
}
