 ...
 * @param {Object} options - projectId, title, body
 */
 ...
 * @param {string} owner - GitHub owner
 * @param {string} projectId - Project ID (required, as positional)
 * @param {string} itemId - Item ID
 */
async function removeItem(owner, projectId, itemId) {
  const args = ['project', 'item-delete', projectId, '--id', itemId, '--owner', owner];
  await execa('gh', args);
}
